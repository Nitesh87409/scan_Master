...
class _TrashScreenState extends State<TrashScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: Text(AppLocalizations.of(context)!.actionRestore),
                onTap: () async {
                  Navigator.pop(context);
                  final success = await _fileManager.restoreFromTrash(file.path);
                  if (success && mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context)!.msgFileRestored)));
                    _loadTrash();
                  }
                },
              ),
              ListTile(
                ...
                title: Text(AppLocalizations.of(context)!.actionDeletePermanently),
                onTap: () async {
                  Navigator.pop(context);
                  await _fileManager.deletePermanently(file.path);
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context)!.msgFileDeleted)));
                    _loadTrash();
                  }
                },
              ),
  ...
  Future<void> _emptyTrash() async {
    final confirm = await AppAnimations.showPremiumDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(AppLocalizations.of(context)!.actionEmptyTrash),
        content: Text('Are you sure you want to permanently delete all files in the trash? This action cannot be undone.'),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: Text(AppLocalizations.of(context)!.actionEmptyTrash),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)!.titleTrashBin),
        actions: [
          if (_trashFiles.isNotEmpty)
            IconButton(
              icon: Icon(Icons.delete_sweep, color: Colors.redAccent),
              onPressed: _emptyTrash,
              tooltip: AppLocalizations.of(context)!.actionEmptyTrash,
            ),
        ],
      ),
      body: _isLoading
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text(AppLocalizations.of(context)!.msgTrashEmpty, style: TextStyle(fontSize: 18, color: Colors.grey.shade600)),
                    ],
                  ),
                )
              : ListView.builder(
                        ...
                        onTap: () {
                          if (file is File) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Cannot open a folder in the trash. Restore it first.')),
                            );
                          }
                        },
                      ),
